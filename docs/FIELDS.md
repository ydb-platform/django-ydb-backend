...
| django.db.models.DecimalField              | Decimal  | Decimal           | Column and bound parameters use the field's `max_digits` / `decimal_places`. Values are rounded to the field's scale; YDB reliably keeps ~26 significant digits. |
...
| django.db.models.JSONField                 | Json     | dict / list / str / int / float / bool | Native YDB `Json` type. Equality filter (`filter(data=value)`) is not supported by YDB. `null=True` is supported (stored as SQL NULL; `__isnull` filtering works). |
